...
- **`idk log` 가 개행 없이 끝난 조각을 이어붙일 때 중복 출력하던 문제** — 초기 tail 이
  ...
  번 이어붙였다. 디스크에서 다시 읽어 오는 경로 하나로 정리했다.
- **`idk log` 가 로테이션 직전 구 파일에 쓰인 마지막 줄을 잃던 문제** — 로테이션(inode
  변경)을 감지하면 새 파일로 곧장 넘어가, 마지막 poll 이후 구 파일에 쓰였지만 아직
  fd 로 읽지 않은 바이트를 그대로 버렸다. 새 파일로 넘어가기 전에 구 fd 를 EOF 까지
  드레인한다.
