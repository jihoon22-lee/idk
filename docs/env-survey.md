# 폐쇄망 환경 조사 — 기존 Python 구현의 조사 기록

> **v0.4 실기는 [새 수용 원장](acceptance/v0.4.0.md#공개-후-사용자-실기-인계)을 따른다.**
> 사용자가 릴리스된 결과물을 현지에서 검증하며 원본 증거는 폐쇄망에 보관한다.
> 아래 과거 양식의 원문·경로 옮겨 적기 지시는 반출 권한을 의미하지 않는다.
> 외부에는 정책상 허용된 판정만 기록하고 확인할 수 없으면 미확인으로 남긴다.

**전제: 파일 반출이 불가능하다.** `doctor --json` 을 떠서 diff 하는 방법을 쓸 수 없으므로,
현지 화면으로 확인하고 외부에 공유 가능한 판정 범위는 폐쇄망 정책을 따른다.
이하 항목과 예시는 기존 Python 구현의 현지 점검용이며 출력 원문의 외부 전달을 요구하지 않는다.
...
```bash
idk --version          # idk 0.3.1 이 나오면 런처 성공
```

| 결과 | 의미 | 적어올 것 |
|---|---|---|
| `idk 0.3.1` | 런처 정상 | "OK" 한 마디면 충분 |
