...
| `config.py` | TOML 로드/저장과 엄격한 타입 helper. 없는 파일은 빈 dict | `config_file()`이 일반 파일 여부를 공통 분류한다. 없는 경로만 missing이고, 디렉터리/FIFO/끊긴 심볼릭 링크/접근 오류는 `ConfigError`다. 저장은 임시파일 → `os.replace` 로 원자적 |
...
| `mirror/model.py` | `mirror.toml`의 artifactory/base_url/auth/token_env 검증과 요청 인증 값 해석 | `base_url`은 공백·userinfo·잘못된 percent escape가 없는 유효한 hostname/port의 HTTP(S)만 허용하고, token_env가 있으면 유효한 bearer 값이 필수다. 토큰·거부된 URL은 모델·출력에 저장하지 않는다 |
