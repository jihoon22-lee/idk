...
호스트는 subreaper로 동작하며 셸 leader의 exit를 WNOWAIT로 관찰한다. SID 재사용을 막기 위해
leader를 바로 reap하지 않고, 새로 확인한 같은 세션의 자식이 모두 사라진 뒤 회수한다. 일반 종료는
실제 자식 정리까지 Closing으로 남고 force는 확인된 대상만 개별 신호로 종료한다. 별도 setsid로
분리된 프로세스는 종료 대상으로 삼지 않는다. 최종 PTY 출력은 최대 2초 동안 drain하고, EOF 없이
수집을 끝내면 화면에 잘림 가능성을 표시한다. 셸 exit와 전체 프로세스 정리를 같은 것으로 보고하지 않는다. 검색은 보관된 표시 행 단위의
