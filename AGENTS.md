...
- 구현·실행 검증·폐쇄망 실기 수용을 구분한다. **2026-09-07 사용자 결정:** 개발·CI·패키지 검증 후
  v0.4를 릴리스하고, 사용자가 공개 결과물로 실기 테스트한다. 폐쇄망 실기는 릴리스 후 후속 작업으로
  추적하며 미실행으로 남긴다. 로컬·CI·패키지의 필수 실패는 공개 blocker다.
  이슈 등록이나 mock PASS를 실제 환경 PASS로 바꾸지 않는다.
