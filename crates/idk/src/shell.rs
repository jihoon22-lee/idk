...
impl PreparedShell {
    pub fn state(&self) -> Result<InitializationState> {
        match fs::read_to_string(&self.state_path)?.trim() {
            ...
            other => bail!("unrecognized shell initialization state: {other:?}"),
        }
    }
}
...
impl ShellPlan {
    ...
    pub fn prepare(&self, resource_root: &Path, launcher: &Path) -> Result<PreparedShell> {
        ...
        let result = self.prepare_in(&resource_dir, launcher, home);
        if result.is_err() {
    ...
    // A bounded, non-startup probe handles /bin/csh symlinks and actual csh
    // binaries without inferring implementation from an executable filename.
    fn is_tcsh(&self) -> Result<bool> {
        let mut child = Command::new(&self.shell)
            .args(["-f", "-c", "echo $?tcsh"])
            .env_clear()
            .envs(&self.env)
            .current_dir(&self.init_cwd)
            .stdin(Stdio::null())
            .stdout(Stdio::piped())
            .stderr(Stdio::null())
            .spawn()
        ...
        loop {
            if let Some(status) = child.try_wait()? {
                ...
                return match output.trim() {
                    "1" => Ok(true),
                    "0" => Ok(false),
                    _ => anyhow::bail!("selected shell is not recognized as csh/tcsh"),
                };
            }
    ...
    fn prepare_in(
        ...
        let bsd_login = self.login && !self.is_tcsh()?;
        ...
        // builtin names are treated as external programs by BSD csh. The random
        // binding is kept below BSD csh's 20-character variable-name limit.
        // tcsh version's lf option means system and user login files precede rc.
        // BSD csh has neither $version nor the lf build option.
        script.push_str("$__idk_builtin[4] __idk_login_first = 0\nif ($?tcsh) then\nif (\"$version\" =~ *\" lf,\"* || \"$version\" =~ *\",lf,\"* || \"$version\" =~ *\",lf\") $__idk_builtin[4] __idk_login_first = 1\nendif\n");
        if self.login {
        ...
        check_status(&mut script, &state);
        script.push_str("endif\n");
        script.push_str("if (! $?command) $__idk_builtin[8]\n");
        ...
        for path in &self.sources {
            ...
            script.push_str(&format!("$__idk_builtin[1] {}\n", quote_path(&resolved)?));
            check_status(&mut script, &state);
        }
        script.push_str(&format!(
            "$__idk_builtin[7] {}\n",
