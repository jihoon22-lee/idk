...
3. fixture, 실제 csh/PTY/Git, WSL/Linux 패키지, 대상 RHEL·폐쇄망 수용을 나눠 판정한다.
   필수 항목의 실패·미실행·확인 불가는 blocker로 남긴다. 테스트 수나 이슈 종료율로 대신하지 않는다.
...
5. 결과는 `R/S | WP/PR | SHA/후보 | 환경·검사 | 결과·제한 | 다음 조치`로 필요한 만큼 정리한다.
   증거가 없는 칸은 미실행/확인 불가로 남긴다. 필수 증거가 없으면 최종 수용을 보류하되
   독립적으로 진행할 수 있는 작업을 제시한다. 읽기 전용 검토는 요청한 답변으로 충분하며
